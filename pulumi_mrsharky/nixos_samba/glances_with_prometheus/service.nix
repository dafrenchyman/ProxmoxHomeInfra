...
let
    ...
    glances_with_prometheus = pkgs.callPackage ./default.nix {};

    # An object containing user configuration (in /etc/nixos/configuration.nix)
    cfg = config.services.glances_with_prometheus;

in {
    # Create the main option to toggle the service state
    options.services.glances_with_prometheus.enable = lib.mkEnableOption "glances_with_prometheus";
    ...
    options.services.glances_with_prometheus.host = lib.mkOption {
        type = lib.types.str;
        default = "0.0.0.0";
        example = "127.0.0.1";
    };
    options.services.glances_with_prometheus.port = lib.mkOption {
        type = lib.types.int;
        default = 9091;
    };
